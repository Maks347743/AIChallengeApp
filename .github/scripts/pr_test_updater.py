...
def analyze_and_generate_tests(
    ...
    system_prompt = (
        ...
        "  - To test that truncation occurs: pass a very large maxLength like 100_000 — larger than any real result.\n"
        "    Assert result shouldContain the known input string (e.g. a unique globalPrefix value).\n"
        "    Do NOT assert result.length <= maxLength — that is trivially true and tests nothing.\n"
        "  - To test that truncation happens: pass maxLength = 50 (a small number).\n"
        "    Assert ONLY result.length shouldBe 50. Do NOT assert shouldContain on any string,\n"
        "    because the truncated result will consist entirely of template content you cannot predict.\n\n"
