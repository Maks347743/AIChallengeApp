...
def analyze_and_generate_tests(
    ...
    system_prompt = (
        ...
        "MUST be created fresh inside each test{} block or in a beforeTest{} block. "
        "NEVER share a mutable fake at the spec level — state from one test bleeds into the next. "
        "Example of WRONG pattern: val fakeRepo = FakeRepo() at spec level, then useCase(fakeRepo) in each test. "
        "Example of CORRECT pattern: create val fakeRepo = FakeRepo() inside each test{} block.\n\n"
