...
def analyze_and_generate_tests(
    ...
    system_prompt = (
        ...
        "- CRITICAL: fake/stub instances that hold mutable state (e.g. a fake repository with var fields) "
        "MUST be created fresh inside each test{} block. "
        "NEVER share a mutable fake at the spec level — state from one test bleeds into the next.\n"
        "- When defining a local fake class inside a test{} block, use plain `class`, NEVER `inner class`. "
        "`inner class` is only valid inside a class body, not inside a lambda, and will cause a compile error.\n"
        "CORRECT pattern:\n"
        "  test(\"example\") {\n"
        "      class FakeRepo : MyRepository {\n"
        "          var lastValue: String? = null\n"
        "          override suspend fun save(s: String) { lastValue = s }\n"
        "      }\n"
        "      val repo = FakeRepo()\n"
        "      val useCase = MyUseCase(repo)\n"
        "      useCase(\"hello\")\n"
        "      repo.lastValue shouldBe \"hello\"\n"
        "  }\n\n"
        ...
        "  - To test that NO truncation occurs: pass maxLength = 100_000 (one hundred thousand).\n"
        "    100_000 is MANDATORY — do not use 1000 or 10000, the system prompt templates alone exceed those sizes.\n"
        "    Assert result shouldContain the known input string (e.g. a unique globalPrefix value).\n"
        "  - To test that truncation DOES happen: pass maxLength = 50 (a small fixed number).\n"
        "    Assert ONLY result.length shouldBe 50. Do NOT assert shouldContain on any string —\n"
        "    the truncated result will consist entirely of template content you cannot predict.\n"
        "  - NEVER write `maxLength = result.length` — `result` does not exist yet at that point; "
        "it is a circular forward reference and will not compile.\n\n"
        ...
        "  import io.kotest.matchers.nulls.shouldBeNull    → value.shouldBeNull()\n"
        "  import io.kotest.matchers.nulls.shouldNotBeNull → value.shouldNotBeNull()\n"
        "  import io.kotest.matchers.string.shouldContain    → str shouldContain substring\n"
        "  import io.kotest.matchers.string.shouldNotContain → str shouldNotContain substring\n"
        "  import io.kotest.matchers.string.shouldBeEmpty    → str.shouldBeEmpty()\n"
        "  import io.kotest.matchers.collections.shouldBeEmpty    → list.shouldBeEmpty()\n"
        "  import io.kotest.matchers.collections.shouldHaveSize   → list shouldHaveSize n\n"
